...
async function handleEvents(event) {
    // 1) If it’s a message event, we can safely check event.message
    if (event.type === 'message') {
        // Check the subtype of the message
        if (event.message.type === 'text') {
            // ------------- TEXT MESSAGE LOGIC -------------
            const userMessage = event.message.text;

            if (userMessage.startsWith("fps:")) {
                let fpsValue = userMessage.split(":")[1]?.trim(); 
                let parsedFps = parseInt(fpsValue, 10);

                if (!isNaN(parsedFps) && parsedFps >= 0 && parsedFps <= 120) {
                    ffmpegConfig.fps = parsedFps;
                    return client.replyMessage(event.replyToken, [
                        { "type": "text", "text": `✅ FPS set to ${ffmpegConfig.fps}` }
                    ]);
                } else {
                    return client.replyMessage(event.replyToken, [
                        { "type": "text", "text": "⚠️ Invalid FPS value! Please enter a number between 0 and 120." }
                    ]);
                }
            }
            ...
            if (userMessage.startsWith("resolution:")) {
                ...
                if (isValidFloat || isValidResolutionFormat) {
                    ffmpegConfig.resolution = isValidFloat
                        ? floatResolution
                        : `${match[1]}x${match[2]}`; // Convert * to x if needed
                    return client.replyMessage(event.replyToken, [
                    ...
                    return client.replyMessage(event.replyToken, [
                        { 
                            "type": "text", 
                            "text": "⚠️ Invalid resolution! Use a value between 0.1 and 1, or a format like 1920x1080 or 1920*1080." 
                        }
                    ]);
                }
            }
            ...
            if (userMessage.startsWith("quality:")) {
                let qualityValue = userMessage.split(":")[1]?.trim().toLowerCase();
            
                if (["low", "standard", "high"].includes(qualityValue)) {
                    ffmpegConfig.quality = qualityValue;
                    return client.replyMessage(event.replyToken, [
                        { "type": "text", "text": `✅ Quality set to ${ffmpegConfig.quality}` }
                    ]);
                } else {
                    return client.replyMessage(event.replyToken, [
                        { "type": "text", "text": "⚠️ Invalid quality! Choose 'low', 'standard', or 'high'." }
                    ]);
                }
            }

            if (userMessage.startsWith("videoOutput:")) {
                let videoOutputValue = userMessage.split(":")[1]?.trim().toLowerCase(); 
                ...
                if (["true", "1"].includes(videoOutputValue)) {
                    ...
                    return client.replyMessage(event.replyToken, [
                        { "type": "text", "text": "⚠️ Invalid videoOutput! Use 'true' or 'false' (or '1'/'0')." }
                    ]);
                }
            
                return client.replyMessage(event.replyToken, [
                    { "type": "text", "text": `✅ videoOutput set to ${ffmpegConfig.videoOutput}` }
                ]);
            }

            if (userMessage.startsWith("outputFormat:")) {
                let formatValue = userMessage.split(":")[1]?.trim().toLowerCase();
                ...
                return client.replyMessage(event.replyToken, [
                    { "type": "text", "text": `✅ outputFormat set to ${ffmpegConfig.outputFormat}` }
                ]);
            }
...
After setting everything, just upload your file and the bot will process it!`;
            ...
            if (event.message.contentProvider.type === 'line') {
                ...
                try {
                    ...
                    annihilateFile(dlpath); // remove original file
                }
            }
        ...
        // No return here; we've handled all message subtypes above.

    // 2) If it’s a postback event
        ...
        // If it didn't match any known postback data:
        return client.replyMessage(event.replyToken, [
            {
                "type": "text",
    ...
    // 3) If user just added or removed the bot
    } else if (event.type === 'follow') {
        // user added bot as friend
        console.log("User followed the bot:", event.source.userId);
        // optional: greet or do nothing
        return;

    } else if (event.type === 'unfollow') {
        // user removed bot from friend list
        console.log("User unfollowed the bot:", event.source.userId);
        // optional: do some cleanup or do nothing
        return;

    // 4) Any other event types
    } else {
        // For events like 'join', 'leave', or unknown, do nothing or handle them if needed
        return;
    }
}     
